...
def demean(series, window, method="simple", min_periods=None):
  if not min_periods:
    min_periods = window
  return series - rolling_mean(series, window, method=method, min_periods=min_periods)
...
class Backtest:
  ...
  def run(self):
    ...
    for col in list(self.wts):
      ...
      self.wts[col] = tmp.pipe(omit_leading_zeros).pipe(fill_date, True)
...
def gen_rtn_component(bt_obj, spot_rtn, carry_rtn, tc = False, main="Returns by Spot/Carry", compound=True):
  ...
  comb = pd.concat([total_component, spot_component, carry_component], axis=1).pipe(omit_leading_na, "any").pipe(omit_leading_zeros)
...
def gen_col_rolling_hitrate(bt_obj, window=252, tc=False):
  returns = getattr(bt_obj, "asset_rtn_tc" if tc else "asset_rtn")
  nrows = len(list(returns))//3 + 1
  gs = GridSpec(nrows = nrows, ncols = 3)
  fig = plt.figure(figsize=(20, nrows * 4.5))
  fig.suptitle("{}-day Rolling Hit Rate".format(str(window)), fontsize=30, y=.925)

  for x, col in enumerate(list(returns)):
    fig.add_subplot(gs[x]).set_title(col).set_label(gen_rolling_hitrate(returns[col], window=window, main=col))
...
def gen_signal(bt_obj, main="Signal Heatmap", show_to=True):
  ax = plt.gca()
  signal = bt_obj.wts.pipe(omit_leading_na, "any").pipe(omit_leading_zeros)
  res = sns.heatmap(
          signal.T.fillna(0),
          alpha=1.0,
          center=0.0,
  ...
  ax.set_yticks(np.arange(.5, len(list(signal)), 1))
  ax.set_yticklabels(list(signal), rotation=0)
  ax.set_yticks(np.arange(0, len(list(signal)), 1), minor=True)

  years = [x.year for x in signal.index]
  ...
  ax.set_xlabel('Year')
  if show_to:
    main = main + " | TO: " + str(round(np.mean(abs(bt_obj.wts.diff(1)).sum(axis=1)) * 252,2))
  ax.set_title(main)
  return ax
...
def plot_bt(bt_obj, t="Full Backtest", compound=True, second_p="2012"):
  ...
  fig.add_subplot(gs[1]).set_label(gen_plot_rtn(bt_obj.model_rtn[second_p:], main = "Returns ({}/)".format(second_p), compound=compound))
...
def plot_bt2(bt_obj, t="Full Backtest", compound=True, second_p="2012"):
  gs = GridSpec(nrows = 3, ncols = 4)
  fig = plt.figure(figsize=(34, 17))
  fig.suptitle(t=t, fontsize=30, y=.925)
